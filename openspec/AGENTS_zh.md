...
针对使用 OpenSpec 进行规范驱动开发的 AI 编码助手的说明。

## TL;DR 快速检查清单

- **搜索现有工作**: `openspec spec list --long`, `openspec list` (仅使用 `rg` 进行全文搜索)
- **确定范围**: 新能力 vs 修改现有能力
- **选择唯一的 `change-id`**: 烤串命名法 (kebab-case)，动词开头 (`add-`, `update-`, `remove-`, `refactor-`)
- **脚手架生成**: `proposal.md`, `tasks.md`, `design.md` (仅在需要时), 以及受影响能力的增量 spec
...
- "Help me plan a change" (帮我计划一个变更)
...
2. 选择一个唯一的以动词开头的 `change-id`，并在 `openspec/changes/<id>/` 下生成 `proposal.md`, `tasks.md`, 可选的 `design.md`, 和 spec 增量文件。
3. 使用 `## ADDED|MODIFIED|REMOVED Requirements` 起草 spec 增量，每个需求至少包含一个 `#### Scenario:`。
...
4. **按顺序实施任务** - 按顺序完成
5. **确认完成** - 在更新状态之前，确保 `tasks.md` 中的每一项都已完成
6. **更新检查清单** - 完成所有工作后，将每个任务设置为 `- [x]` 以反映实际情况
7. **批准门槛** - 在提案经过审查和批准之前，不要开始实施

### 第三阶段：归档变更
部署后，创建单独的 PR 以：
- 移动 `changes/[name]/` → `changes/archive/YYYY-MM-DD-[name]/`
- 如果能力发生变化，更新 `specs/`
- 对于仅工具相关的更改，使用 `openspec archive <change-id> --skip-specs --yes` (总是显式传递 change ID)
...
- [ ] 阅读 `openspec/project.md` 获取约定
- [ ] 运行 `openspec list` 查看活跃的变更
- [ ] 运行 `openspec list --specs` 查看现有的能力

**在创建规范之前:**
- 始终检查能力是否已经存在
- 优先修改现有规范而不是创建重复项
- 使用 `openspec show [spec]` 审查当前状态
- 如果请求模棱两可，在生成脚手架之前先问 1-2 个澄清问题
...
│   │   └── specs/          # 增量变更
│   │       └── [capability]/
│   │           └── spec.md # ADDED/MODIFIED/REMOVED (新增/修改/移除)
│   └── archive/            # 已完成的变更
```
...
1. **创建目录:** `changes/[change-id]/` (烤串命名法, 动词开头, 唯一)
...
### 需求措辞
- 对规范性需求使用 SHALL/MUST (避免使用 should/may，除非有意非规范性)
...
- ADDED: 引入一个新的能力或子能力，可以作为需求独立存在。当变更是正交的（例如，添加“Slash Command Configuration”）而不是改变现有需求的语义时，首选 ADDED。
- MODIFIED: 更改现有需求的行为、范围或验收标准。始终粘贴完整的、更新后的需求内容（标题 + 所有场景）。归档程序将用您在此处提供的内容替换整个需求；部分增量将丢失之前的细节。
- RENAMED: 仅在名称变更时使用。如果您也更改了行为，请使用 RENAMED (名称) 加上 MODIFIED (内容) 引用新名称。

常见陷阱：使用 MODIFIED 添加新关注点而不包括以前的文本。这会导致在归档时丢失细节。如果您没有显式更改现有需求，请在 ADDED 下添加新需求。

正确编写 MODIFIED 需求：
1) 在 `openspec/specs/<capability>/spec.md` 中找到现有需求。
2) 复制整个需求块（从 `### Requirement: ...` 到其场景）。
3) 粘贴到 `## MODIFIED Requirements` 下并进行编辑以反映新行为。
...
- 使用烤串命名法 (kebab-case)，简短且描述性强: `add-two-factor-auth`
- 优先使用动词开头的操作前缀: `add-`, `update-`, `remove-`, `refactor-`
